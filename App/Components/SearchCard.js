...
const SearchCard = ({ item, onPress }) => {
  ...
  return (
    <View
      style={apply(
        'bg-white border-1 border-primary-color rounded-14 mx-2 overflow-hidden mb-2'
      )}
    >
        ...
        <View style={apply('p-3 h-168 w-271')}>
          <View style={apply('row items-start')}>
            ...
            <View style={apply('ml-3')}>
              <Text
                style={apply('text-primary-color font-bold text-title mb-1')}
              >
                {item.title}
              </Text>
              <Text
                style={apply(
                  'font-bold text-secondary-color text-caption mb-2'
                )}
              >
                {item.company}
              </Text>
              <View style={apply('row')}>
                {positions.map((position) => (
                  <View
                    key={position._id}
                    style={apply('mr-2 bg-primary-color rounded-lg p-1')}
                  >
          ...
          >
            <Text style={apply('text-primary-color text-caption flex')}>
              {item.location}
            </Text>
            <Text style={apply('text-primary-color text-caption')}>
